...
This requires the `gh` CLI (GitHub's official CLI tool) which handles enterprise SSO authentication automatically. On first run, this downloads `bootstrap.mk` into `.harness.mk` and clones the full framework into `.harness-framework/`. Both are gitignored. Subsequent runs pull updates in the background without blocking your build.
...
```env
# Jira (Server/DC uses bearer auth with PAT, Cloud uses basic auth with email+token)
